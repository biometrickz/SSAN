...
class Normaliztion_val_video(object):
    """
...
class Normaliztion_val_video_ImageNet(object):
    """
        same as mxnet, normalize into [-1, 1]
        image = (image - 127.5)/128
    """
...
class ToTensor_val_video(object):
    """
...
def transformer_val_video():
    return transforms.Compose([Normaliztion_val_video(), ToTensor_val_video()])

# use ImageNet mean and var for normalization
def transformer_val_video_ImageNet():
    return transforms.Compose([ToTensor_val_video(), Normaliztion_val_video_ImageNet()])

def transformer_valcustom():
    return transforms.Compose([ToTensor_val_video(), Normaliztion_val_video()])